...
######################################################################
#   this is im=9 in nimble_models_1to13.R
#   without accounting for uncertainty with W and ww
######################################################################
    im <- 2
    code[[im]] <- nimbleCode({
        #  model for the LTLA data
...
######################################################################
#   Model 2 with BYM on the LTLA-specific random effects
######################################################################
    im <- 4
    code[[im]] <- nimbleCode({
        #  model for the LTLA data
    ...
    code[[im]] <- nimbleCode({
        ...
        for (i in 1:nareas) {
            for (t in 1:ntimes) {
                ...
                mu_pv[i,t] ~ dnorm(m[i,t],sd=sd_pv)  #  Type I space-time interaction
                m[i,t] <- u[i] + b[t] + (gamma[i]+mu_a)*(w[i,t]-8) + 
                          beta.bame*std_bame[i] + beta.imd*std_imd[i]
            }
            u[i] ~ dnorm(alpha,sd=sd_u)
            gamma[i] ~ dnorm(0,sd=sd_gamma)
        }
...
get_final_data <- function(imodel,fitdata) {
    ...
    if (imodel==5) not_use <- c(not_use,'imd','bame')
